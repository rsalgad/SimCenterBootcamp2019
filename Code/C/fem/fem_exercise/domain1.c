...
void domainAddConstraint(Domain *theDomain, int nodeTag, int constraints[]){

  Node* node = domainGetNode(theDomain, nodeTag);
  for (int i = 0; i < 3; i++){
    if (constraints[i] == 1)
...
void domainAddLoad(Domain *theDomain, int nodeTag, double loads[]){

  Node* node = domainGetNode(theDomain, nodeTag);
  for (int i = 0; i < 3; i++){
    if (loads[i] != 0)
      node->loads[i] = loads[i];
  }
}
